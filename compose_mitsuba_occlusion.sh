...
python blender_datagen_compose.py \
  ...
  glbs_placement_bbox=[-2.8,-2.0,2.8,2.0] \
  placement_bbox=[-3.1,-2.3,3.1,2.3] \
  glbs_scale_range=[1,3] \
  radius_range=[5.6,6.6] \
  fov_range=[35,50] \
  placement_bbox_scale=0.75 \
  placement_grid_res=[160,128] \
  camera_object_clearance=0.2 \
  camera_sample_retry_limit=60 \
  glbs_z_offset_range=[0.0,0.0] \
  glbs_max_sample_tries_per_scene=2000 \
  scene_compose_retry_limit=60 \
  enclosure.enabled=true \
  enclosure.ceiling=false \
  enclosure.height=6.0 \
  enclosure.floor_material=diffuse \
